...
interface Graph extends Map<string, Set<string>> {
    clusters: Map<string, { x: number; z: number }>
}
 ...
 */
export function findEnclosedPolygons(walls: WallState[]): string[][] {
 ...
 */
function buildGraph(walls: WallState[]): Graph {
    const graph = new Map() as Graph

    const getPosKey = (x: number, z: number) => `${x.toFixed(2)},${z.toFixed(2)}`
    ...
    for (let i = 0; i < allPoints.length; i++) {
        ...
        const cluster: WallPoint[] = [allPoints[i]]
        assigned.add(i)
...
function findAllCycles(graph: Graph): string[][] {
    ...
    function dfs(node: string, start: string, parent: string | null) {
 ...
 */
export function generateRoofGeometry(polygon: string[], graph: Graph): THREE.BufferGeometry | null {
 ...
 */
function getCentroid(positions: [number, number][]): [number, number] {
 ...
 */
export function getRoofPolygons(walls: WallState[] | Map<string, WallState>): RoofPolygon[] {
    ...
    }).filter((p): p is RoofPolygon => p !== null)
}
