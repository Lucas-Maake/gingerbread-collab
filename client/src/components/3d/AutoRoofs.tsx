...
import type { RoofStyle } from '../../context/gameStore'
...
const ROOF_THICKNESS = ROOF.THICKNESS
const ROOF_OVERHANG = ROOF.OVERHANG
const ROOF_EMISSIVE = '#4a3020'

// Calculate pitched roof height based on angle and average roof span
function calculateRoofHeight(vertices: [number, number][], angleDegrees: number) {
 ...
 */
function RoofPolygon({ vertices, roofStyle, pitchAngle }: RoofPolygonProps) {
    ...
    const geometry = useMemo(() => {
        ...
        const expandedVertices = vertices.map(([x, z]) => {
            ...
            if (dist < 0.01) return [x, z] as [number, number]
            const scale = (dist + ROOF_OVERHANG) / dist
            return [centerX + dx * scale, centerZ + dz * scale] as [number, number]
        })
