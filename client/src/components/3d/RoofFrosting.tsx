 ...
 */
function IcicleDrip({ position, length }: { position: [number, number, number], length: number }) {
    return (
 ...
 */
function FrostingEdge({ start, end, center }: FrostingEdgeProps) {
    const { drips, tubePoints } = useMemo(() => {
        ...
        const dripList: { position: [number, number, number], length: number, key: string }[] = []
...
export default function RoofFrosting() {
    ...
    const edges = useMemo(() => {
        ...
        roofPolygons.forEach((roof, roofIndex) => {
            ...
            const center: [number, number] = [centerX, centerZ]
